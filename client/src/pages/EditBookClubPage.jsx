...
function EditBookClubPage() {
  ...
  useEffect(() => {
    const fetchBookClub = async () => {
      try {
        const res = await api.get(`/bookclubs/${id}`);
        const { name, description, coverImage, createdBy } = res.data;
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.put(`/bookclubs/${id}`, { name, description, coverImage });
      navigate(`/bookclubs/${id}`);
    } catch (error) {
      console.error('Failed to update book club', error);
    } finally {
      setLoading(false);
    }
  };
