...
function BookResultCard({ book, shelfStatus, onShelfChange }) {
  ...
  return (
    <Card 
      ...
      sx={{ borderRadius: '16px', textDecoration: 'none', height: '100%', display: 'flex', flexDirection: 'column' }}
    >
      <CardMedia 
        component="img" 
      ...
      <Box sx={{ flexGrow: 1 }} /> 
      <CardActions sx={{ pl: 1, pb: 1 }}>
        <Button variant={buttonContent.variant} color={buttonContent.color} size="small" onClick={handleClick} sx={{width: '100%'}} startIcon={shelfStatus ? <CheckCircleOutlineIcon /> : null}>
          {buttonContent.text}
        </Button>
